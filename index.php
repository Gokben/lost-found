...
$retentionStmt = db()->prepare('SELECT setting_value FROM settings WHERE setting_key=?');
$retentionStmt->execute(['retention_days']);
$retentionDays = max(1, (int)($retentionStmt->fetchColumn() ?: 90));
...
<?php foreach($items as $item):
  ...
  $expiryDate = $foundDate->setTime(0, 0)->modify('+' . $retentionDays . ' days');
