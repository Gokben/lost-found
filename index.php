...
if ($q !== '') {
    ...
    foreach($terms as $term){$sql.=' AND ('.implode(' OR ',array_map(fn($column)=>'i.'.$column.' LIKE ?',$searchColumns)).')';$like='%'.$term.'%';foreach($searchColumns as $_)$params[]=$like;}
}
if ($status !== '') {
    $sql .= ' AND status=?';
    $params[] = $status;
