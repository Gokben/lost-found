...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$error) {
        $sql = 'UPDATE items SET item_no=?,serial_no=?,related_items=?,found_at=?,location=?,found_department=?,found_by=?,quantity=?,category=?,name=?,brand=?,color=?,details=?,storage_location=?,status=?,delivered_at=?,delivery_method=?,delivered_by=?,delivery_form_no=?,recorded_by=? WHERE id=?';
        $values = [trim($_POST['item_no']),trim($_POST['serial_no'] ?? ''),trim($_POST['related_items'] ?? ''),$foundAt,$location,trim($_POST['found_department']),trim($_POST['found_by'] ?? ''),max(1,(int)($_POST['quantity'] ?? 1)),$category,$name,trim($_POST['brand'] ?? ''),trim($_POST['color'] ?? ''),trim($_POST['details'] ?? ''),trim($_POST['storage_location']),trim($_POST['status']),$deliveredAt,trim($_POST['delivery_method'] ?? ''),trim($_POST['delivered_by'] ?? ''),trim($_POST['delivery_form_no'] ?? ''),$_SESSION['user']['name'],$id];
        try {
...
<header><a class="brand brand-back-link" href="<?=url('index.php')?>"><img class="entry-brand-logo" src="<?=url('assets/kirpisoftware-logo-transparent-v2.png')?>" alt="Kirpisoft"><span><b>Lost &amp; Found</b></span></a><a class="home-link" href="<?=url('index.php')?>" title="Ana sayfa">⌂</a></header>
...
<label class="details-field">Açıklama<textarea name="details" rows="2"><?=field('details')?></textarea></label><div class="item-images-field">Görseller<div class="item-image-uploader" data-image-upload data-existing="<?=count($images)?>"><label class="item-image-upload-button">Görsel Ekle<input type="file" name="item_images[]" accept="image/jpeg,image/png,image/webp,image/gif" multiple></label><div class="item-image-preview"><?php foreach($images as $image):?><label class="item-image-tile"><img src="<?=url($image['image_path'])?>" alt="Eşya görseli"><input type="checkbox" name="remove_image_ids[]" value="<?=(int)$image['id']?>"><button class="item-image-remove" type="button" onclick="this.parentElement.classList.toggle('marked');this.parentElement.querySelector('input').checked=this.parentElement.classList.contains('marked')">×</button></label><?php endforeach?></div><span class="item-image-count"><?=count($images)?>/4 görsel</span><span class="item-image-help">En fazla 4 görsel, her biri en çok 5 MB.</span></div></div><div class="add-related-item"><a class="button new-button" href="<?=url('item-new.php?parent_id='.$id)?>">Eşya Ekle</a></div>
</div></section><div class="entry-actions"><a class="button secondary" href="<?=url('index.php')?>">Vazgeç</a><a class="button new-button" target="_blank" href="<?=url('item-label.php?id='.$id)?>">Etiket Yazdır</a><button class="save-button" type="submit">Kaydet</button></div></form></main>
<script src="<?=url('assets/required.js')?>"></script><script src="<?=url('assets/item-images.js')?>"></script></body></html>
