...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$error) {
        ...
        $sql = 'UPDATE items SET item_no=?,serial_no=?,related_items=?,found_at=?,location=?,found_department=?,found_by=?,quantity=?,category=?,name=?,brand=?,color=?,details=?,storage_location=?,status=?,delivered_at=?,delivery_method=?,delivered_by=?,delivery_form_no=?,updated_by=?,updated_at=? WHERE id=?';
...
if ($readOnly) echo '<style>#item-form input,#item-form select,#item-form textarea,#item-form button{pointer-events:none}#item-form .save-button,#item-form .add-related-action,#item-form .item-images-field{display:none!important}</style><script>document.addEventListener("DOMContentLoaded",()=>{const meta=document.querySelector(".record-meta-field");if(meta){meta.style.setProperty("grid-column","3 / -1","important");meta.style.setProperty("grid-row","4","important");meta.style.setProperty("align-self","stretch","important")}})</script>';
?>
...
<label>Bulunduğu yer *<select name="location" id="location-select" required><option value="">Yer seçiniz</option><?php foreach($locations as $locationRow):?><option <?=selected($locationRow['name'],$selectedLocation)?>><?=e($locationRow['name'])?></option><?php endforeach?></select></label><label id="room-number-field" hidden>Oda numarası *<input name="room_number" inputmode="numeric" pattern="\d{3,4}" maxlength="4" list="existing-room-numbers" value="<?=e($roomValue)?>" placeholder="Mevcut oda seçin"><datalist id="existing-room-numbers"><?php foreach($roomNumbers as $room):?><option value="<?=e($room)?>"><?php endforeach?></datalist></label><label>Bulunduğu tarih *<span class="fixed-date-field"><input id="found-at" type="text" inputmode="numeric" pattern="\d{2}\.\d{2}\.\d{4}" placeholder="gg.aa.yyyy" name="found_at" value="<?=e($_POST['found_at'] ?? date_display($item['found_at']))?>" required><button class="fixed-date-button" type="button" data-date-picker="found-at-picker" aria-label="Bulunduğu tarih için takvimi aç"><svg viewBox="0 0 24 24" aria-hidden="true"><path d="M7 2v3m10-3v3M4 9h16M5 5h14a1 1 0 0 1 1 1v13a1 1 0 0 1-1 1H5a1 1 0 0 1-1-1V6a1 1 0 0 1 1-1Zm3 8h2m2 0h2m2 0h2m-8 3h2m2 0h2"/></svg></button><input id="found-at-picker" class="native-date-source" type="date" value="<?=e(date_input_to_storage($_POST['found_at'] ?? $item['found_at'])??'')?>" tabindex="-1" aria-hidden="true"></span></label>
