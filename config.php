...
define('BASE_PATH', $configuredBasePath !== false ? $configuredBasePath : ($isLocalHost ? '' : '/lostfound'));

const DB_HOST = 'localhost';
const DB_NAME = 'lostfound';
const DB_USER = 'lostfound_user';
...
function db(): PDO
{
    ...
    if (!$pdo) {
        if (getenv('APP_ENV') === 'local') {
            $pdo = new PDO('sqlite:' . __DIR__ . '/storage/local.sqlite');
